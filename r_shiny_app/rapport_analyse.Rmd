...
# Paramètres interactifs du rapport
params:
  departement_choisi:
    label: "Département :"
    value: "Les deux"
...
kpi_ges_total <- paste0(
  format(round(mean(data_filtree$emission_ges_5_usages), 0), big.mark = " "), " kgCO₂/an"
)
...
table_ges <- df2 %>%
  ...
  group_by(departement) %>%
  summarise(
    GES_total = paste0(round(mean(emission_ges_5_usages),1)," kgCO₂/an"),
...
if (length(unique(data_filtree$departement)) > 1) {
  ...
  # Cas simple : un seul département
  data_bar <- data_filtree %>%
    filter(cout_total_5_usages < quantile(cout_total_5_usages, 0.99, na.rm = TRUE)) %>%
...
df_plot_periode <- data_filtree %>%
  filter(!is.na(periode_construction)) %>%     #  suppression dess NA
  count(periode_construction) %>%
  mutate(
    pourcentage = n / sum(n),
    periode_construction = factor(
      periode_construction,
      levels = unique(periode_construction)   
    )
  )
