...
for (code_commune_insee in codes_insee) {
    ...
    while (has_data) {
      ...
      if (status_code(response) == 200) {
        ...
        if (!is.null(content$results) && is.data.frame(content$results) && nrow(content$results) > 0) {
          ...
          df_final_neufs <- bind_rows(df_final_neufs, df)
  ...
  message("✅ Fin du code INSEE : ", code_commune_insee, 
          " | Total cumulé : ", nrow(df_final_neufs), " lignes.")
  
  # --- Sauvegarde intermédiaire ---
  write.csv(df_final_neufs, "resultats_dpe_neufs_savoie.csv", row.names = FALSE)
}

# --- Résumé final ---
message("🟢 Extraction terminée : ", nrow(df_final_neufs), " lignes au total.")
View(df_final_neufs)
