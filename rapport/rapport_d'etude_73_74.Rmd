...
load_report_data <- function() {
  ...
  df_existants <- read.csv("data_dpe_2_savoies_existants.csv",
                           sep = ";", dec = ".", fileEncoding = "UTF-8")
  
  df_neufs <- read.csv("data_dpe_2_savoies_neufs.csv",
                       sep = ";", dec = ".", fileEncoding = "UTF-8")
...
Dans le cadre du défi proposé par Enedis sur l’évaluation de l’impact du Diagnostic de Performance Énergétique (DPE) sur les consommations des logements, la société GreenTech Solutions nous a confié la réalisation d’une étude statistique ainsi qu’une application interactive développée sous RShiny.

Ce rapport constitue le premier livrable du projet. Il présente une analyse structurée des performances énergétiques des logements situés en Savoie (73) et en Haute-Savoie (74), à partir des données publiques des DPE « logements neufs » et « logements existants », enrichies par la Base Adresse Nationale.

Étant donné le volume très important des données initiales, un échantillonnage maîtrisé a été réalisé :

un échantillon fixe de 10 000 adresses issues de la BAN ;

un échantillon de 10% des DPE neufs et existants.
Cette réduction de volume permet d’assurer de bonnes performances dans l’application Shiny tout en préservant la représentativité statistique des distributions observées.

L’étude poursuit deux objectifs principaux :

décrire et comparer les profils énergétiques des deux départements ;

identifier les facteurs influençant les coûts énergétiques, en particulier l’étiquette DPE, la période de construction et la surface habitable.

Pour répondre à ces objectifs, l’analyse s’appuie sur :

des KPI clés (coûts, émissions GES, surfaces, répartition des classes DPE),

des visualisations variées (histogrammes, barres, comparaisons départementales, nuages de points),

des statistiques bivariées, dont la corrélation entre surface habitable et coût de chauffage,

ainsi que des analyses interactives intégrées dans l’application Shiny.

Les résultats présentés mettent en lumière les tendances générales observées sur le territoire, ainsi que les spécificités propres à chaque département.
...
moy_surface <- mean(data_filtree$surface_habitable_logement, na.rm = TRUE)

kpi_surface_moyenne <- if (is.na(moy_surface)) {
  "—"
} else {
  paste0(round(moy_surface, 1), " m²")
}
